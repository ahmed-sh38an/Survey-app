<template>
  <PageComponent title="Dashboard">
    ...
    >
      ...
      <div
        class="bg-white shadow-md p-4 row-span-2 animate-fade-in-down order-3 lg:order-1"
      >
        ...
        <img
          ...
          alt=""
        />
        <h3 class="text-xl font-bold mb-3">{{ data.latestSurvey.tittle }}</h3>
        ...
        <div class="flex justify-between text-sm mb-1">
          <div>Questions:</div>
          <div>{{ data.latestAnswers.questions }}</div>
        </div>
        <div class="flex justify-between text-sm mb-3">
          <div>Answers:</div>
          <div>{{ data.latestAnswers.answers }}</div>
        </div>
        <div class="flex justify-between">
          <router-link
            :to="{ name: 'SurveyView', params: { id: data.latestSurvey.id } }"
            class="flex py-2 px-4 border border-transparent text-sm rounded-md text-indigo-500 hover:text-white transition-colors hover:bg-indigo-700 focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
          >
            <svg
      ...
      <div
        class="bg-white shadow-md p-4 row-span-2 animate-fade-in-down order-4 lg:order-3"
